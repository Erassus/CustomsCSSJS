...
function cssjs($action='ossn') {
	...
	case 'edit':
		...
		foreach($fields as $field){
			...
			$content = input("{$field}");
			if ($OssnComponents->setComSETTINGS('CustomsCSSJS', $field, $content)){
				ossn_trigger_message(ossn_print('com:customs:cssjs:update', array($label)), 'success');
			} else {
				ossn_trigger_message(ossn_print('com:customs:cssjs:update:fail', array($label)), 'error');
			}
		}	
		redirect(REF);
	}
